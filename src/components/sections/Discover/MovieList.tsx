...
const MovieDiscoverList = () => {
  ...
  const fetchMovies = async ({ pageParam = 1 }: { pageParam: number }) => {
    ...
    const queryMap: Record<string, () => Promise<any>> = {
      todayTrending: () => tmdb.trending.trending('movie', 'day'),
      thisWeekTrending: () => tmdb.trending.trending('movie', 'week'),
      popular: () => tmdb.movies.popular(pageParam),
      nowPlaying: () => tmdb.movies.nowPlaying(pageParam),
      upcoming: () => tmdb.movies.upcoming(pageParam),
      topRated: () => tmdb.movies.topRated(pageParam),
    };
    const fn = queryMap[queryType];
    if (!fn) return tmdb.discover.movie({ sort_by: 'popularity.desc', page: String(pageParam) });
    return fn();
  };
  ...
  if (status === 'pending') {
    return <Spinner size="lg" className="absolute-center mt-20" variant="simple" color="primary" />;
  }
  ...
  return (
    <>
      ...
      <div ref={ref} className="flex h-24 items-center justify-center">
        {isFetchingNextPage && (
          <Spinner size="lg" variant="wave" label={getLoadingLabel()} color="primary" />
        )}
