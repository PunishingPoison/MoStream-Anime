...
const TvShowHomeList: React.FC<QueryList<any>> = ({ query, name, param }) => {
  ...
  return (
    <section id={key} className="min-h-[200px] overflow-hidden" ref={ref}>
      {isPending ? (
        <TvShowHomeListSkeleton />
      ) : (
        <div className="z-3 flex flex-col gap-3">
          <div className="flex items-center justify-between gap-4">
            <div className="min-w-0 shrink">
              <SectionTitle color="warning" className="truncate">{name}</SectionTitle>
            </div>
            <Link size="sm" href={`/discover?type=${param}&content=tv`} isBlock color="foreground" className="shrink-0 flex items-center gap-1.5 rounded-full text-sm font-medium text-muted-foreground transition-all duration-200 hover:text-warning hover:gap-3">
              See All
              <span className="text-warning transition-transform duration-200">&rarr;</span>
            </Link>
          </div>
          <Carousel>
