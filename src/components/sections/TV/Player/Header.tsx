...
const TvPlayerHeader: React.FC<TvPlayerHeaderProps> = ({ id, tvName, season, episode, onOpenSource, hidden }) => {
  return (
    <div
      className={cn(
    ...
    >
      <div className="flex items-center gap-3">
        <Button isIconOnly size="sm" variant="light" className="text-white bg-white/10 backdrop-blur-sm hover:bg-white/20" as={Link} href={`/tv/${id}`}>
          <IoChevronBack size={24} />
        </Button>
        <span className="text-sm font-semibold text-white drop-shadow-sm truncate max-w-[200px] md:max-w-md">
          {tvName} - S{season}:E{episode}
        </span>
      </div>
      <Button
        size="sm"
