  ...
  return (
    <>
      <Tooltip
        ...
        content={<HoverPosterCard id={movie.id} type="movie" />}
      >
        <Link href={`/movie/${movie.id}`} ref={ref} {...longPress()}>
          {variant === 'full' && (
            <div className="group relative aspect-2/3 overflow-hidden rounded-lg border-[2px] border-transparent text-foreground netflix-card hover:border-primary/50 hover:shadow-2xl hover:shadow-primary/30 hover:-translate-y-1">
              <div className={cn(
          ...
          {variant === 'bordered' && (
            <Card isHoverable fullWidth shadow="sm" className="group h-full border border-white/10 bg-secondary-background transition-all duration-300 hover:-translate-y-1.5 hover:border-primary/40 hover:shadow-xl hover:shadow-primary/20">
      ...
      {mobile && (
        <VaulDrawer
          ...
          hiddenTitle
        >
          <HoverPosterCard id={movie.id} type="movie" fullWidth />
        </VaulDrawer>
      )}
    </>
  );
