...
interface MovieSeasonsSelectionProps {
  id: number;
  seasons: any[];
}

const MovieSeasonsSelection = forwardRef<HTMLDivElement, MovieSeasonsSelectionProps>(
  ({ id, seasons }, ref) => {
    ...
    const { data: seasonData } = useQuery<any>({
      queryFn: () => tmdb.movies.seasonDetails(id, activeSeason),
      queryKey: ['movie-season-detail', id, activeSeason],
    ...
    return (
      <section id="seasons" ref={ref} className="relative z-3">
        <SectionTitle color="primary">Episodes</SectionTitle>
        <div className="mt-4">
                  ...
                  <Button
                    ...
                    className={
                      activeSeason === season.season_number
                        ? 'font-semibold shadow-lg shadow-primary/30 scale-105'
        ...
        <div className="mt-4 flex flex-col gap-4">
          {seasonData?.episodes?.map((episode: any) => (
            <div key={episode.id} className="group flex gap-4 rounded-xl bg-secondary-background/50 p-3 transition-all duration-300 hover:bg-secondary-background hover:ring-1 hover:ring-primary/20 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/20">
              ...
              <div className="flex min-w-0 flex-1 flex-col gap-1.5">
                <div className="flex items-start justify-between gap-2">
                  <h4 className="truncate text-sm font-bold md:text-base">
                    <span className="text-muted-foreground">Ep.{episode.episode_number}</span> {episode.name}
                  </h4>
                  {episode.air_date && (
                ...
                <div className="mt-auto flex items-center gap-2 pt-1">
                  <Button
                    as={Link}
                    href={`/movie/${id}/player?season=${episode.season_number}&episode=${episode.episode_number}`}
                    size="sm"
                    color="primary"
                    variant="flat"
                    startContent={<FaCirclePlay size={12} />}
                    className="font-semibold transition-all duration-200 hover:scale-105 hover:shadow-lg hover:shadow-primary/20"
                  >
                    Play Episode
                  </Button>
                </div>
...
MovieSeasonsSelection.displayName = 'MovieSeasonsSelection';
export default MovieSeasonsSelection;
