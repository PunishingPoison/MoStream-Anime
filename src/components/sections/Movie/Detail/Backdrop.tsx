...
const BackdropSection: React.FC<{ movie: any }> = ({ movie }) => {
  const [{ y }] = useWindowScroll();
  const opacity = Math.min((y / 800) * 2, 1);
  const backdropImage = getImageUrl(movie?.backdrop_path, 'backdrop');
  const titleImage = movie?.images?.logos
    ? getImageUrl(movie.images.logos.find((logo: any) => logo.iso_639_1 === 'en')?.file_path, 'title')
    : '';
